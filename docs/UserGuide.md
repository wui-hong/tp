...
| Email address&ast; | e | Valid email format required: `local-part@domain`. The local-part must be alphanumeric or the following special characters: `+`, `-`, `.`, `_`. Additionally, it cannot start or end with a special character. The domain is made of labels separated by periods. A domain must end with a label at least 2 characters long. A domain label's characters must be alphanumeric or a hyphen. Furthermore, labels cannot start or end with a hypen | `damithch@comp.nus.edu.sg`, `dcsdcr@nus.edu.sg` |
